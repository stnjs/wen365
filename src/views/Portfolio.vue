        ...
        <div class="flex justify-between items-center h-16">
          <div class="flex items-center space-x-8">
            ...
            <nav class="hidden md:flex space-x-6">
              ...
              <router-link to="/portfolio" class="text-primary-600 font-medium">Portfolio</router-link>
      ...
      <div v-else>
        ...
        <div class="card">
          ...
          <div v-if="filteredAssets.length === 0" class="text-center py-12">
            ...
            <p class="text-sm text-gray-500">Connect your wallet to see your portfolio</p>
          </div>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <div class="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center mr-4">
                        <span class="text-primary-600 font-semibold">{{ asset.token.symbol.slice(0, 2) }}</span>
                      </div>
                      <div>
                        <div class="font-medium text-gray-900">{{ asset.token.symbol }}</div>
                        <div class="text-sm text-gray-500">{{ asset.token.name }}</div>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-medium text-gray-900">{{ formatBalance(asset.balance, asset.token.decimals) }}</div>
                    <div class="text-sm text-gray-500">{{ asset.token.symbol }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-medium text-gray-900">${{ formatNumber(asset.balanceUsd) }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">{{ asset.averageHoldingDays }} days</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      ...
                      <span class="text-sm text-gray-900">{{ asset.taxFreePercentage.toFixed(1) }}%</span>
                    </div>
                  </td>
...
const filteredAssets = computed(() => {
  ...
  return assets.value.filter(asset => 
    asset.token.symbol.toLowerCase().includes(searchTerm.value.toLowerCase()) ||
