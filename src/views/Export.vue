  ...
  <div class="min-h-screen bg-gray-50">
        ...
        <div class="flex justify-between items-center h-16">
          <div class="flex items-center space-x-8">
            ...
            <nav class="hidden md:flex space-x-6">
              ...
              <router-link to="/export" class="text-primary-600 font-medium">Export</router-link>
            </nav>
          </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="card">
          <h2 class="text-xl font-semibold text-gray-900 mb-6">Export Configuration</h2>
            ...
            <div>
              ...
              <div class="space-y-2">
                <label class="flex items-center">
                  ...
                  >
                  <span class="ml-2 text-sm text-gray-700">Tax-free assets</span>
                </label>
                <label class="flex items-center">
                  <input 
                ...
                <label class="flex items-center">
                  <input 
                    ...
                    class="rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                  >
                  <span class="ml-2 text-sm text-gray-700">Transaction history</span>
                </label>
              </div>
            </div>
        ...
        <!-- Export Preview -->
        <div class="card">
          <h2 class="text-xl font-semibold text-gray-900 mb-6">Export Preview</h2>
          
          <div class="space-y-4">
            <div class="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
              <div>
                <div class="font-medium text-gray-900">Total Assets</div>
                <div class="text-sm text-gray-500">{{ assets.length }} unique tokens</div>
              </div>
              <div class="text-right">
                <div class="font-medium text-gray-900">${{ formatNumber(totalValue) }}</div>
              </div>
            </div>

            <div class="flex justify-between items-center p-4 bg-crypto-green/10 rounded-lg">
              <div>
                <div class="font-medium text-gray-900">Tax-Free Assets</div>
                <div class="text-sm text-gray-500">{{ taxFreeAssets.length }} assets</div>
              </div>
              <div class="text-right">
                <div class="font-medium text-crypto-green">${{ formatNumber(taxFreeValue) }}</div>
              </div>
            </div>

            <div class="flex justify-between items-center p-4 bg-crypto-red/10 rounded-lg">
              <div>
                <div class="font-medium text-gray-900">Taxable Assets</div>
                <div class="text-sm text-gray-500">{{ taxableAssets.length }} assets</div>
              </div>
              <div class="text-right">
                <div class="font-medium text-crypto-red">${{ formatNumber(taxableValue) }}</div>
              </div>
            </div>
            ...
            <div class="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
              <div>
                ...
                <div class="text-sm text-gray-500">{{ transactions.length }} total</div>
              </div>
              <div class="text-right">
                <div class="font-medium text-gray-900">{{ transactions.length }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="mt-6">
            ...
            <div v-else class="space-y-2">
              ...
              >
                <div>
                  <div class="font-medium text-gray-900">{{ export.filename }}</div>
                  <div class="text-sm text-gray-500">{{ formatDate(export.timestamp) }}</div>
                </div>
...
const exportHistory = ref<Array<{id: string, filename: string, timestamp: number}>>([])
